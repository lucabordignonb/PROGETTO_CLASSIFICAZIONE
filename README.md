...
2. **Scarica e analizza il modello preesistente**:
   ...
   - Questo modello è stato eseguito e commentato dettagliatamente nel file **MOD_ORIG_COMM.ipynb** per comprenderne il funzionamento.
...
4. **Esegui i file principali del progetto**:
   - **File 1 (Codice Commentato)**: Analizza il modello preesistente con commenti dettagliati.
     ```bash
     jupyter notebook MOD_ORIG_COMM.ipynb
     ```
   - **File 2 (Modello con Prestazioni Non Buone)**: Mostra le performance di un modello iniziale.
     ```bash
     jupyter notebook Mod2_ieri_accnonbuona.ipynb
     ```
   - **File 3 (Modello Migliorato)**: Presenta il modello ottimizzato con tecniche avanzate.
     ```bash
     jupyter notebook Ultimaproiva_perf_top.ipynb
     ```

### Documentazione

Il progetto è suddiviso in tre file principali, che rappresentano le diverse fasi di analisi e sviluppo. L'obiettivo principale è confrontare un modello preesistente con due modelli sviluppati durante il progetto, evidenziando i punti di forza, i limiti e le differenze tra le implementazioni.

1. **MOD_ORIG_COMM.ipynb**:
...
3. **Ultimaproiva_perf_top.ipynb**:
   - Contiene un modello migliorato con l'inclusione di tecniche avanzate come data augmentation, bilanciamento del dataset e ottimizzazione dei parametri.
   - Sebbene migliori rispetto al modello precedente (File 2), l'obiettivo è capire le differenze rispetto al modello iniziale basato su EfficientNet (File 1).
...
Il progetto mira a:
- Analizzare e comprendere il funzionamento del modello preesistente.
- Confrontare le performance dei modelli sviluppati internamente con il modello originale.
- Valutare quale modello sia più efficace per la classificazione dei loghi, identificando i punti di forza e le aree di miglioramento di ciascuna implementazione.
...
2. **Attiva l'ambiente su Python**:
   Dopo aver creato l'ambiente lo abbiamo attivato con il comando "conda activate"
...
- **MOD_ORIG_COMM.ipynb**: Codice commentato di un modello preesistente basato su EfficientNet, pubblicato su Kaggle. Questo file analizza e documenta il funzionamento e le scelte implementative del modello originale.
- **Mod2_ieri_accnonbuona.ipynb**: Modello iniziale sviluppato durante il progetto. Questo modello mostra prestazioni di base ed evidenzia le limitazioni di un'implementazione senza ottimizzazioni avanzate.
- **Ultimaproiva_perf_top.ipynb**: Modello migliorato con tecniche avanzate come data augmentation e bilanciamento del dataset. È utilizzato per confrontare le performance rispetto al modello iniziale e a quello preesistente.
